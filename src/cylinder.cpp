...
void Cylinder::setSections ( double distance )
{
...
void Cylinder::residue ( lmx::Vector<double>& res, lmx::Vector<double>& conf )
{
...
void Cylinder::jacobian ( lmx::Matrix<double>& jac, lmx::Vector<double>& conf )
{
    double t = conf.readElement ( 0 );
    jac.writeElement (
...
void Cylinder::computeIntersection ( Particle* particle )
{
    x = particle->getX();
    y = particle->getY();
    z = particle->getZ();
    ...
    lmx::NLSolver<Cylinder> theSolver;
    theSolver.setInfo ( 0 );
    theSolver.setInitialConfiguration ( initialGuess );
    theSolver.setSystem ( *this );
    theSolver.setResidue ( &Cylinder::residue );
    theSolver.setJacobian ( &Cylinder::jacobian );
//   theSolver.setConvergence( &Cylinder::myConvergence );
