        ...
        , ogive_radius ( par4_in )
{
    ...
    double tau = asin ( length/ogive_radius );
    final_radius = initDiam/2. - ogive_radius* ( 1. - cos ( tau ) );
}
...
void Ogive::setSections ( double distance )
{
...
void Ogive::computeGeometry()
{
    ...
    {
        z = i * sectionDif + z0;
        tau = asin ( ( z-z0 ) /ogive_radius );
        radius = initDiam/2. - ogive_radius* ( 1. - cos ( tau ) );
//     radius = (length + z0 - z) * tan(slope);
        for ( j=0; j<sectors; ++j )   //closed chain
        {
            theta = 2 * pi * j / sectors - pi; // angle between -pi and pi
            x = radius * cos ( theta );
            y = radius * sin ( theta );
    ...
    theta = 2 * pi / sectors; // angle between -pi and pi

    for ( i=0; i<sections-1; ++i )
    {
        zlocal = i * sectionDif;
        tau = asin ( zlocal/ogive_radius );
        ra = initDiam/2. - ogive_radius* ( 1. - cos ( tau ) );
        tau = asin ( zlocal/ogive_radius );
        rb = initDiam/2. - ogive_radius* ( 1. - cos ( tau ) );
...
void Ogive::residue ( lmx::Vector<double>& res, lmx::Vector<double>& conf )
{
    ...
    /*  if ( ( pow(y+z0*vy,2)+pow(x+z0*vx,2) > pow(initDiam/2.,2) ) ||
           ( pow(y+(z0+length)*vy,2)+pow(x+(z0+length)*vx,2) <
               pow(final_radius,2) )
         )
        res.writeElement(0.,0);*/
//   else if(t<z0 ){
//       res.writeElement( pow(x+vx*t,2)+pow(y+vy*t,2) - initDiam/2., 0);
//   }
//   else{
//     double tau = asin((t-z0)/ogive_radius);
//     double radius = initDiam/2. - ogive_radius*(1. - cos(tau));

    res.writeElement (
//                      (pow(x+vx*t,2)+pow(y+vy*t,2) - radius)
        -initDiam/2.+ogive_radius* ( 1.-sqrt ( 1-pow ( t-z0,2 ) /pow ( ogive_radius,2 ) ) )
        +sqrt ( pow ( y+t*vy,2 ) +pow ( x+t*vx,2 ) )
        , 0
    );
//   }
}

void Ogive::jacobian ( lmx::Matrix<double>& jac, lmx::Vector<double>& conf )
{
    double t = conf.readElement ( 0 );

//   if(t<z0 /*|| t>(z0+length)*/)
//     jac.writeElement( 2*vy*(y+t*vy)+2*vx*(x+t*vx)
//         , 0, 0);
//   else{
//    double tau = asin((t-z0)/ogive_radius);
//    double d_tau_dt =1./(ogive_radius*sqrt(1-pow(t-z0,2)/pow(ogive_radius,2)));
    jac.writeElement
    ( ( t-z0 ) / ( ogive_radius*sqrt ( 1-pow ( t-z0,2 ) /pow ( ogive_radius,2 ) ) )
      + ( 2*vy* ( y+t*vy ) +2*vx* ( x+t*vx ) ) / ( 2*sqrt ( pow ( y+t*vy,2 ) +pow ( x+t*vx,2 ) ) )
      , 0, 0
    );
//   }
}

void Ogive::computeIntersection ( Particle* particle )
{
    x = particle->getX();
    y = particle->getY();
    z = particle->getZ();
    ...
    initialGuess ( 0. ) = z0;
    lmx::NLSolver<Ogive> theSolver;
    theSolver.setInfo ( 0 );
    theSolver.setInitialConfiguration ( initialGuess );
    theSolver.setSystem ( *this );
    theSolver.setResidue ( &Ogive::residue );
    theSolver.setJacobian ( &Ogive::jacobian );
//   theSolver.setConvergence( &Ogive::myConvergence );
//   theSolver.setMaxIterations( 100 );
    if ( ( pow ( y+z0*vy,2 ) +pow ( x+z0*vx,2 ) > pow ( initDiam/2.,2 ) ) /* ||
         ( pow(y+(z0+length)*vy,2)+pow(x+(z0+length)*vx,2) <
         pow(final_radius,2) )*/
       )
        paramTrajectories.push_back ( z0-1. );
    else
    {
        theSolver.solve ( 100 );
...
void Ogive::computeNodalPower ( Particle* particle )
{
