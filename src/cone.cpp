...
void Cone::setSections ( double distance )
{
...
void Cone::residue ( lmx::Vector<double>& res, lmx::Vector<double>& conf )
{
...
void Cone::jacobian ( lmx::Matrix<double>& jac, lmx::Vector<double>& conf )
{
    double t = conf.readElement ( 0 );
    jac.writeElement (
...
void Cone::computeIntersection ( Particle* particle )
{
    x = particle->getX();
    y = particle->getY();
    z = particle->getZ();
    ...
    lmx::NLSolver<Cone> theSolver;
    theSolver.setInfo ( 0 );
    theSolver.setInitialConfiguration ( initialGuess );
    theSolver.setSystem ( *this );
    theSolver.setResidue ( &Cone::residue );
    theSolver.setJacobian ( &Cone::jacobian );
