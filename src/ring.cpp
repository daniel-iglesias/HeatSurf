...
void Ring::setSections ( double distance )
{
    sections = ( extDiam-intDiam ) /distance + 1 ;
    std::cout << "Sections = " << sections << std::endl;
}


void Ring::computeGeometry()
{
    int i, j;
    double pi = 3.1416;
    double theta;
    double radius, ra, rb;
    double a, b, c;
    double x, y, z ( z0 ), zlocal ( 0. );
    double sectionDif = ( extDiam-intDiam ) / ( sections - 1 );

    // nodes map is going to be filled to a size of i*j where
    // i is the number of radial sections and
    // j is the number of transversal (angular) sectors.
    for ( i=0; i<sections; ++i )
    {
        radius = ( intDiam/2. ) - i*sectionDif;
...
void Ring::jacobian ( lmx::Matrix<double>& jac, lmx::Vector<double>& conf )
{
    double t = conf.readElement ( 0 );
    jac.writeElement ( 0
...
void Ring::computeIntersection ( Particle* particle )
{
    x = particle->getX();
    y = particle->getY();
    z = particle->getZ();
    ...
    lmx::NLSolver<Ring> theSolver;
    theSolver.setInfo ( 0 );
    theSolver.setInitialConfiguration ( initialGuess );
    theSolver.setSystem ( *this );
    theSolver.setResidue ( &Ring::residue );
    theSolver.setJacobian ( &Ring::jacobian );
//   theSolver.setConvergence( &Ring::myConvergence );
...
void Ring::computeNodalPower ( Particle* particle )
{
...
//     cout << "paramTrajectories.back() = " << paramTrajectories.back() << endl;
        ...
        double particle_power = particle->getEnergy() * 0.128678;
